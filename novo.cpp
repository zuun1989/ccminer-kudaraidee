...
int scanhash_novo(int thr_id, struct work* work, uint32_t max_nonce, unsigned long* hashes_done)
{

	uint32_t* pdata = work->data;
	uint32_t* ptarget = work->target;
	const uint32_t first_nonce = pdata[19];
	uint32_t throughput = cuda_default_throughput(thr_id, 1U << 28);
	...
	do
	{
		novo_cpu_hash(thr_id, throughput, pdata[19], ms, pdata[16], pdata[17], pdata[18], work->nonces);
		if(work->nonces[0] != UINT32_MAX)
		{
			uint32_t vhash64[8]={0};

			novo_hash(vhash64, pdata, work->nonces[0], ms);
			if (vhash64[7] == 0 && fulltest(vhash64, ptarget))
			{
				...
				*hashes_done = pdata[19] - first_nonce + throughput;
				...
				{
					novo_hash(vhash64, pdata, work->nonces[1], ms);
					if (vhash64[7] == 0 && fulltest(vhash64, ptarget))
					{
