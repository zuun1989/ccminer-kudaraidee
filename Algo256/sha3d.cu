...
// compat
extern void keccak256_sha3d_init(int thr_id, uint32_t threads);
extern void keccak256_sha3d_free(int thr_id);
extern void keccak256_sha3d_setBlock_80(void *pdata, const void *ptarget);
extern void keccak256_sha3d_hash_80(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t* resNonces, int order);
...
extern "C" int scanhash_sha3d(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (!init[thr_id])
	{
		...
		use_compat_kernels[thr_id] = true;

		if(!use_compat_kernels[thr_id]) {
			keccak256_cpu_init(thr_id);
		} else {
			// really useful ?
			keccak256_sha3d_init(thr_id, throughput);
		}
	...
	if(use_compat_kernels[thr_id])
		keccak256_sha3d_setBlock_80((void*)endiandata, ptarget);
	else {
	...
	do {
		...
		*hashes_done = pdata[19] - first_nonce + throughput;
		...
		if(use_compat_kernels[thr_id])
			keccak256_sha3d_hash_80(thr_id, throughput, pdata[19], work->nonces, order++);
		else {
